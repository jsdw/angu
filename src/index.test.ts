...
describe('index', function(){
    ...
    it('can provide back error information when something goes wrong', () => {
        ...
        const r1 = calcjs.evaluate('10 * 4 + 2', ctx)
    ...
    it('can be used to manipulate cells in a spreadsheet', () => {
        ...
        const r1 = calcjs.evaluate('SUM(A1:A3)', ctx).value
        assert.equal(r1, 6)
        const r2 = calcjs.evaluate('D1 + D2', ctx).value
        assert.equal(r2, 363)
        const r3 = calcjs.evaluate('MEAN(A1:A5) + SUM(C1:D2) - 10', ctx).value
        assert.equal(r3, 412)
